...
This repository contains supplementary computer vision models which can be accessed as [web services](#usage) by PhotoPrism and other applications. They provide a [REST API](#example-request) that accepts an image URL and returns, for example, a matching caption in response. The currently [integrated models](#models) for this, each with [its own endpoint](#api-endpoints), are *kosmos-2*, *blip-image-captioning large* and *vit-gpt2-image-captioning*.
